// Credits to Inigo Quilez's article for inspiration on a lot of the functions : 
...
float shaping_func(ivec2 cell_coord, float x) {
    return    cell_coord==ivec2(0,0) ? smooth_clamp(x, 0.5, 0.15)
            : cell_coord==ivec2(1,0) ? exp_impulse(x, 8.0, 0.2)
            : cell_coord==ivec2(2,0) ? acc_to_const_v(x, 4.0, 2.0)
            : cell_coord==ivec2(3,0) ? sin_bounce(x, 10.0, 0.5)
            ...
            : cell_coord==ivec2(3,2) ? tonemap(x, 2.0)
            : cell_coord==ivec2(0,3) ? gaussian(x, 1.0, 0.5, 5.0)
            : cell_coord==ivec2(1,3) ? cubic_pulse(x, 0.5, 1.0)
            : cell_coord==ivec2(2,3) ? 0.0
            : 1.0;
}

const bool SHOW_ALL_CELLS = true;
